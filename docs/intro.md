...
**Current status:** Phase 3. Spin Wheel and Scratch Card are available as headless engines, React Native hooks, and accessible animated components.
...
All five packages are published. `@jackpotkit/core` contains shared primitives plus the Spin Wheel and Scratch Card engines. `@jackpotkit/react-native` contains their hooks, renderers, providers, and imperative APIs. `@jackpotkit/theme` contains platform-neutral themes, while `@jackpotkit/testing` contains deterministic consumer helpers. The React web renderer remains intentionally empty until its roadmap phase.

Start with [Spin Wheel](./spin-wheel.md), [Scratch Card](./scratch-card.md), or [Core primitives](./core-primitives.md), and review the [server-authoritative result boundary](./server-authoritative-results.md) before integrating rewards with real value.
